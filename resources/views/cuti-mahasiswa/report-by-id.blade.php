...
<head>
    ...
    <title>Surat Cuti Mahasiswa</title>
    <style>
        body {
...
<body>
    ...
    <!-- Judul Laporan -->
    <h1 style="text-align: center; margin-bottom: 10px;"><b>SURAT CUTI MAHASISWA</b></h1>
    <h3 style="text-align: center; margin-top: 0px;"><b><u>{{ $cutiMahasiswa->nomor_surat }}</u></b></h3>
    <div class="content">
        <p>Kepada Yth:
            <br><b>{{ $cutiMahasiswa->mahasiswa->name }}</b>
        </p>
        <p>Sehubungan dengan Pengajuan Cuti yang Saudara ajukan sebelumnya dengan alasan
            <b>{{ $cutiMahasiswa->alasan_cuti }}</b>, dengan tanggal cuti sebagai berikut :
        </p>
        <table class="table">
            <thead>
                <tr>
                    ...
                    <td class="text-left align-middle">
                        {{ \Carbon\Carbon::parse($cutiMahasiswa->tanggal_mulai)->translatedFormat('d F Y') }} -
                        {{ \Carbon\Carbon::parse($cutiMahasiswa->tanggal_selesai)->translatedFormat('d F Y') }}</td>
                </tr>
            </thead>
        </table>
        <p>Berkenaan hal tersebut, maka pengajuan yang saudara ajukan <b>Disetujui</b> oleh
            <b>{{ $cutiMahasiswa->verifier->name }}</b>.
        </p>
