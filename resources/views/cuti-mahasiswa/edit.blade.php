<x-app-layout>
  <x-slot name="header">
    {{ __('Edit Pengajuan Cuti Mahasiswa') }}
  </x-slot>
  ...
  <div class="p-4 bg-white rounded-lg shadow-xs">
    <form action="{{ route('cuti-mahasiswa.update', $cutiMahasiswa->id) }}" method="POST">
      ...
      <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
        ...
        {{-- hidden nomor surat --}}
        <input type="text" name="nomor_surat" value="{{ $cutiMahasiswa->nomor_surat }}">
        ...
        <div class="sm:col-span-2">
          ...
          <textarea name="alasan_cuti" id="alasan_cuti" rows="4"
            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">{{ old('alasan_cuti', $cutiMahasiswa->alasan_cuti) }}</textarea>
          @error('alasan_cuti')
