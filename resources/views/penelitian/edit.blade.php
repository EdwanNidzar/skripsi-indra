<x-app-layout>
  <x-slot name="header">
    {{ __('Edit Pengajuan Izin Penelitian') }}
  </x-slot>
    ...
    <form action="{{ route('penelitian.update', $penelitian->id) }}" method="POST" enctype="multipart/form-data">
      ...
      <div class="sm:col-span-1 mt-2">
        ...
        <input type="text" name="judul" id="judul"
          class="block w-full px-3 py-2 mt-2 text-sm border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
          placeholder="Judul Penelitian" value="{{ old('judul', $penelitian->judul) }}">
        @error('judul')
          <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
        @enderror
      </div>
        ...
        <div class="sm:col-span-1">
          ...
          <select name="lama_penelitian" id="lama_penelitian"
            ...
            <option value="2 bulan"
              {{ old('lama_penelitian', $penelitian->lama_penelitian) == '2 bulan' ? 'selected' : '' }}>2 Bulan</option>
            <option value="3 bulan"
              {{ old('lama_penelitian', $penelitian->lama_penelitian) == '3 bulan' ? 'selected' : '' }}>3 Bulan</option>
            <option value="4 bulan"
              {{ old('lama_penelitian', $penelitian->lama_penelitian) == '4 bulan' ? 'selected' : '' }}>4 Bulan</option>
            <option value="5 bulan"
              {{ old('lama_penelitian', $penelitian->lama_penelitian) == '5 bulan' ? 'selected' : '' }}>5 Bulan
            </option>
            <option value="6 bulan"
              {{ old('lama_penelitian', $penelitian->lama_penelitian) == '6 bulan' ? 'selected' : '' }}>6 Bulan
            </option>
          </select>
