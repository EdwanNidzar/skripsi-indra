<x-app-layout>
  <x-slot name="header">
    {{ __('Tambah Pengajuan Penelitian') }}
  </x-slot>
      ...
      <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
        ...
        <div class="sm:col-span-1">
          <label for="judul" class="block text-sm font-medium text-gray-700">Judul Penelitian</label>
            <input type="judul" name="judul" id="judul" class="block w-full px-3 py-2 mt-1 text-sm border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="Judul Penelitian">
          @error('judul')
            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
          @enderror
        </div>
