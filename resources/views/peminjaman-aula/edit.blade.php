<x-app-layout>
  <x-slot name="header">
    {{ __('Edit Pengajuan Peminjaman Aula') }}
  </x-slot>
  ...
  <div class="p-4 bg-white rounded-lg shadow-xs">
    <form action="{{ route('peminjaman-aula.update', $peminjaman_aula->id) }}" method="POST">
      ...
      <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
        ...
        {{-- hidden nomor surat --}}
        <input type="hidden" name="nomor_surat" value="{{ $peminjaman_aula->nomor_surat }}">
        ...
        <div>
          <label for="nama_penanggung_jawab" class="block text-sm font-medium text-gray-700">Nama Penanggung
            Jawab</label>
        ...
        <div>
          ...
          <input type="text" name="jabatan" id="jabatan" value="{{ old('jabatan', $peminjaman_aula->jabatan) }}"
            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
            required>
          @error('jabatan')
            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
        ...
        <div>
          ...
          <input type="text" name="prodi" id="prodi" value="{{ old('prodi', $peminjaman_aula->prodi) }}"
            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
            required>
          @error('prodi')
            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
        ...
        <div>
          ...
          <input type="text" name="no_hp" id="no_hp" value="{{ old('no_hp', $peminjaman_aula->no_hp) }}"
            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
            required>
          @error('no_hp')
            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
